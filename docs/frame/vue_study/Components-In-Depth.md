...
- 备注：==**props是只读的**==，Vue底层会监测你对props的修改，如果进行了修改，就会发出警告，若业务需求确实需要修改，那么请复制 props 的内容到 data 中，然后去修改 data 中的数据
...
作用：让父组件可以向子组件指定位置<strong style="color:#DD5145">插入 HTML 结构</strong>，也是一种**组件间通信的方式**，适用于 <strong style="color:#DD5145">父组件 ===> 子组件</strong>
