...
**NAT（Network Address Translation）网络地址转换**，通过将私有网络地址转换为公用网络地址，从而对外隐藏内部管理的IP地址。

NAT不仅能解决公网IP地址不足的问题，而且还能够有效地避免来自网络外部的攻击，隐藏并保护网络内部的计算机。
...
**静态NAT**就是一对一映射，内部有多少私有地址需要和外部通信，就要配置多少外网IP地址与其对应，并不节省外网IP，所以一般不用。

![image-20241025143208627](InterviewQuestions.assets/image-20241025143208627.png)

#### 2.动态转换

**动态NAT**是在路由器上配置一个**外网IP地址池**，当内部有计算机需要和外部通信时，就从地址池里动态的取出一个外网IP，并将他们的对应关系绑定到NAT表中，通信结束后，这个外网IP才被释放，可供其他内部IP地址转换使用，这个DHCP租约IP有相似之处。

![image-20241025143221682](InterviewQuestions.assets/image-20241025143221682.png)

#### 3.端口地址转换PAT

**端口地址转换（Port Address Translation，PAT）**，也加端口多路复用。是最常用的NAT技术，它提供了多对一的方式，对多个内网IP地址，边界路由可以给他们分配一个外网IP，利用这个外网IP的不同端口和外部进行通信。

![image-20241025142734014](InterviewQuestions.assets/image-20241025142734014.png)

对于 **NAT** ( **网络地址转换** ) 技术来讲，最重要的一点是，在配置 NAT 的路由器上形成了 NAT 转换表，这个转换表的形成是非常关键的。

> NAT转换表存放着`{本地IP地址:端口} - {公用IP地址:端口}`的映射关系。

### 网络地址端口转换 NAPT

**网络地址端口转换 NAPT（Network Address Port Translation）** 则是把内部地址映射到外部网络的一个IP地址的不同端口上。

NAPT与动态NAT不同，它将内部连接映射到外部网络中的一个单独的IP地址上，同时在该地址上加上一个由NAT设备选定的端口号。
...
>
> NAT路由器在转发IP数据报时，需要转换源IP地址和目的IP地址
...
:book:参考：[NAT 详解](https://www.coonote.com/note/network-address-translation.html)、[一文了解网络地址转换（NAT）](https://blog.csdn.net/weixin_44604541/article/details/118359580)
...
ICMP（Internet Control Message Protocol）<strong style="color:#DD5145">Internet 控制报文协议</strong>，是一种<strong style="color:#DD5145">面向无连接的网络层协议</strong>，是 TCP/IP 协议簇的一个子协议，用于在IP主机、路由器之间传递**控制消息**。

ICMP也有两个版本：**ICMPv4 和 ICMPv6**。

ICMP协议与TCP、UDP等协议不同，<strong style="color:#DD5145">它不用于传输数据，只是用来发送控制消息</strong>。这些控制消息虽然并不传输用户数据，但是对于用户数据的传递起着重要的作用，**可以报告错误信息或者异常情况**。
...
- 第一个功能是**确认IP数据包是否能够成功到达目标地址**
- 第二个功能是进行**网络诊断**，经常使用 ICMP 数据包的两个终端程序是 `ping` 和 `traceroute`

### ICMP的结构

ICMP的报文分为两个部分：**ICMP报文首部**和**ICMP报文数据**，其报文是通过**封装在IP数据报中**进行数据传输的。

![image-20241028145039540](InterviewQuestions.assets/image-20241028145039540.png)
...
已经定义的 ICMP 消息类型大约有10多种，每种ICMP数据类型都被封装在一个IP数据包中。主要的 ICMP 消息类型包括以下几种。这几种消息又主要分为两大类：**差错报告报文**、**查询报文**。

#### 差错报告报文

差错报告报文常见有：**目标不可达（类型3）**、**重定向（类型5）**、**超时消息（类型11）**。

<strong style="color:#DD5145">目标不可达（类型 Type：3）</strong>

| 类型Type | 代码Code | 说明                                                         |
| -------- | -------- | ------------------------------------------------------------ |
| 3        | 0        | Net Unreachable，网络不可达                                  |
| 3        | 1        | Host Unreachable，主机不可达                                 |
| 3        | 2        | Protocol Unreachable，协议不可达                             |
| 3        | 3        | Port Unreachable，端口不可达                                 |
| 3        | 4        | Fragmentation needed but no frag. bit set，<br>需要进行分片，但设置不分片比特 |

- **「网络不可达」**：IP地址可以表示一个网络，当主机号全为0时就表示的是某一个网络，如果整个网络不可达，就会报告一个类型为3，具体代码为0 的 ICMP协议报文。
- **「主机不可达」**：如果主机A要和主机B进行通信，而主机B是关机的状态，就会出现主机不可达的情况，就会报告一个类型为3，具体代码为1 的 ICMP协议报文。

![image-20241028151730119](InterviewQuestions.assets/image-20241028151730119.png)

<strong style="color:#DD5145">重定向（类型 Type：5）</strong>

| 类型Type | 代码Code | 说明                            |
| -------- | -------- | ------------------------------- |
| 5        | 0        | Redirect for net，对网络重定向  |
| 5        | 1        | Redirect for host，对主机重定向 |

- **「网络重定向」**：传输给某一个网络的数据，可能不能走该网络了，需要进行重定向。
- **「主机重定向」**：如果发送的报文，主机告知不能处理，请发送到另外一个主机。

<strong style="color:#DD5145">超时消息（类型 Type：11）</strong>

在IP数据包中有一个叫做`TTL(Time To Live, 生存周期)` ，它的值在经过路由器一跳之后都会减 1，IP数据包的TTL减为 0 时会被丢弃。此时IP路由器会发送一个ICMP超时消息发送给主机，通知该包已经被丢弃。

| 类型Type | 代码Code | 说明                         |
| -------- | -------- | ---------------------------- |
| 11       | 0        | Time Exceeded，超时 TTL值为0 |

> 设置TTL的主要目的是避免IP数据包在存在问题的网络中无休止的在网络上转发。

#### 查询报文

查询报告报文常见有：**回送请求或应答（类型8或0）、时间戳请求或应答（类型13或14）**。

<strong style="color:#DD5145">回送请求或应答（类型 Type：8 或 0）</strong>

| 类型Type | 代码Code | 说明                            |
| -------- | -------- | ------------------------------- |
| 8        | 0        | Echo Request，回送请求 ping请求 |
| 0        | 0        | Echo Reply，回送响应 ping响应   |

- **「回送请求或回送应答」**：主要是验证网络是否通。假设主机A要和主机B进行通信，A会发送一个空的数据给B，如果B收到，就给一个回应。

> 网络中最常用的`ping`命令就是利用这个实现查询和回应的。

<strong style="color:#DD5145">时间戳请求或应答（类型 Type：13 或 14）</strong>
...
<strong style="color:#DD5145">源抑制（类型 4）</strong>

在使用低速率网络的情况下，网络通信可能会遇到网络拥堵的情况下，ICMP 的原点抑制就是为了应对这种情况的。

当路由器向低速线路发送数据时，其发送队列的残存数据报变为 0 从而无法发送时，可以向 IP 数据报的源地址发送一个 `ICMP 原点抑制(ICMP Source Quench Message)` 消息，收到这个消息的主机了解到线路某处发生了拥堵，从而抑制 IP 数据报的发送。
...
- `ping`：用来测试两个主机之间的连通性，测试两台主机之间的连接速度，并准确报告数据包到达目的地并返回后所花费的时间。
- `tracert`：`trace router`的缩写，用来显示到达目的主机的路径，用于显示两台互联网设备之间可能的路径，并测量数据包在 IP 网络上的时延

#### 测试网络通断

使用`ping`检查网络连通性有五个步骤：

1. 使用 `ipconfig /all` 观察本地网络设置是否正确；

2. `Ping 127.0.0.1`，ping 回送地址是为了检查本地的**TCP/IP 协议栈**是否正常工作；

3. `Ping 本机IP地址`，是为了检查**网络适配器**（网络接口卡，NIC）是否正常工作；

   - 如果ping不通，则需要检查本机的网线、网络适配器配置、IP地址有问题；

4. `Ping 本网网关/本网其他IP地址`，是为了检查**硬件设备（网关路由器）**是否正常，也可以检查**本机与本地网络连接**是否正常通信；（在非局域网中这一步骤可以忽略)

5. `Ping 远程IP地址`，这主要是检查**本网或本机与外部**的连接是否正常。

:book:参考：​[网络诊断工具：ICMP、Ping 和 Traceroute 的原理与应用](https://juejin.cn/post/7426923218952962100?searchId=20241028163752C8423D2084ACC699CAE9)
...
:book:参考文章：[Ping 的工作原理你懂了，那 ICMP 你懂不懂？](https://www.cnblogs.com/cxuanBlog/p/14445393.html)、[参考文章](https://aws.amazon.com/cn/what-is/icmp/)、[网络层-ICMP协议](https://juejin.cn/post/6854573211598716941?searchId=20241028143616346EF08D6C5D737FDF9A)、[参考](https://www.cnblogs.com/beyondsec/p/17923117.html)
...
路由信息协议，RIP（Routing Information Protocol）是基于<strong style="color:#5090FF">距离矢量(向量)算法</strong>的路由协议，利用 **跳数(hop count)** 作为计量标准。
...
OSPF 是一个**内部网关协议 (IGP)**，在自治系统（autonomous system，AS）内决策路由。RIP 也是一个内部网关协议。
...
OSPF 使用的是<strong style="color:#5090FF">分布式链路状态协议</strong>，而非像 RIP 那样的距离向量协议。和 RIP 协议相比，OSPF 主要有下面这些变化：
...
**边界网关协议（Border Gateway Protocol，BGP）**是运行于 TCP 上的一种自治系统的路由协议。<strong style="color:#5090FF">BGP的主要作用是实现自治系统之间的路由选择和路径选择</strong>。

> BGP 是唯一一个用来处理像因特网大小的网络的协议，也是唯一能够妥善处理好不相关路由域间的多路连接的协议。

在 RIP 和 OSPF 这些协议中，是由 IP 的网络地址进行路由控制，而 BGP 协议则需要通过整个互联网进行路由控制，范围要大很多。

> <strong style="color:#DD5145">应用层协议，基于TCP协议</strong>传输数据（端口号为179）。

#### BGP 协议特点

1. BGP是外部路由协议，用在AS之间传递信路由信息。
2. BGP支持CIDR（无分类域间路由），因此 BGP 的路由表也就应当包括当前目的网络前缀、下一跳路由器，以及到达目的网络所经过的 AS 。
3. 在BGP刚运行时，BGP 的邻站交换整个 BGP 路由表，但以后只需要在发生变化时更新有新变化的部分即可（这样节省网络带宽和减少路由器开销有很大好处）。
4. BGP是一种增强的距离矢量路由协议，从设计上避免了环路的发生。
5. 周期性发送 KeepAlive（保活）报文以保持 TCP连通性。
...
:book:参考文章：[图解路由协议！](https://www.cnblogs.com/cxuanBlog/p/16325607.html)、[6 分钟看完 BGP 协议。](https://www.cnblogs.com/cxuanBlog/p/16442198.html)、[万字详解BGP路由技术原理及配置命令！](https://cloud.tencent.com/developer/article/1838022)
...
TCP 叫做<strong style="color:#DD5145">传输控制协议(TCP，Transmission Control Protocol)</strong>，通过名称可以大致知道 TCP 协议有控制传输的功能，主要体现在其可控，可控就表示着可靠，确实是这样的，TCP 为应用层提供了一种**可靠的、面向连接**的服务，它能够将分组可靠的传输到服务端。

UDP 叫做<strong style="color:#DD5145">用户数据报协议(UDP，User Datagram Protocol)</strong>，通过名称可以知道 UDP 把重点放在了数据报上，它为应用层提供了一种无需建立连接就可以直接发送数据报的方法。

> 我们上面提到会将运输层的分组称为报文段，除此之外，还会将 TCP 中的分组也称为报文段，然而将 UDP 的分组称为数据报，同时也将网络层的分组称为数据报

## :star:TCP 和 UDP? 区别? 应用场景?

### 1.TCP

TCP（Transmission Control Protocol），传输控制协议，是一种**可靠、面向字节流的通信协议**，把上面应用层交下来的数据看成无结构的字节流来发送。
...
- TCP 是面向连接的协议，建立连接三次握手、断开连接四次挥手。
...
DNS（Domain Name System）即域名系统，是互联网一项服务，它作为将域名和IP地址相互映射的一个分布式数据库，能够使人更方便地访问互联网。<strong style="color:#DD5145">DNS 使用 UDP 端口 53</strong>。
...
除此之外，还有电脑默认的**本地域名服务器**

<strong style="color:#5090FF">根域: 全球根服务器节点只有13个,10个在美国，1个荷兰，1个瑞典，1个日本</strong>

### DNS服务器类型

DNS服务器用于域名和IP地址的转换，具有层次结构，包括根域名、顶级域名和本地域名服务器。域名解析过程涉及递归查询和迭代查询，用于从域名找到相应的IP地址。

<strong style="color:#DD5145">主域名服务器</strong>：管理和维护所负责解析的域内解析库的服务器

<strong style="color:#DD5145">从域名服务器</strong>：从主服务器或从服务器 “复制”（区域传输）解析库副本

<strong style="color:#DD5145">缓存域名服务器</strong>：只提供域名解析结果的缓存功能，目的在于提高查询速度和效率，但是没有自己控制的区域地址数据。构建缓存域名服务器时，必须设置根域或指定其他DNS服务器作为解析来源

<strong style="color:#DD5145">转发域名服务器</strong>：负责所有非本地域名的本地查询。转发域名服务器接到查询请求后，在其缓存中查找，如找不到就将请求依次转发到指定的域名服务器，直到查找到结果为止，否则返回无法映射的结果。

:book:参考：[DNS服务器 - 理论](https://blog.csdn.net/u010230019/article/details/130195777)



### DNS查询类型

DNS 查询类型有两种：递归查询、迭代查询

![image-20241024165955310](InterviewQuestions.assets/image-20241024165955310.png)

#### 递归查询

主机向本地域名服务器的查询一般都采用**递归查询**（recursive query ）。

当主机向本地域名服务器发出请求后，若本地域名服务器本身不能解析，则会向其他根域名服务器发出查询请求（即**替该主机继续查询**），直到查询到结果后，再层层传递回来。

此查询的源和目标保持不变，为了查询结果只需要发起一次查询。（不需要自己动手，它去给你找）

#### 迭代查询

本地域名服务器向根域名服务器的查询通常采用**迭代查询**（iterative query ）。

当根域名服务器收到本地域名服务器发送的迭代查询请求报文时，要么给出所要查询的IP地址，要么告诉本地服务器下一步应当向哪一个域名服务器进行查询，然后让本地域名服务器进行后续查询。逐步按照域树的路径向下走直到叶节点，得到了所要解析的IP地址，然后把这个结果返回给发起查询的主机

此查询的源不变，但查询的目标不断变化，为查询结果一般需要发起多次查询。(需要自己动手，你去哪里找)
...
1. 首先查询**浏览器缓存**，缓存中维护一张域名与 IP 地址的对应表
2. 若没有命中，则继续查询**操作系统的DNS缓存**，一般在`hosts`文件中
3. 若没有命中，则主机到本地域名服务器进行查询，本地域名服务器采用递归查询，查找成功则返回结果
4. 若没有命中，则本地域名服务器向**根域名服务器**进行迭代查询
   - 首先本地域名服务器向根域名服务器发起请求，根域名服务器**返回顶级域名服务器的地址**给本地服务器。
   - 本地域名服务器根据这个顶级域名服务器的地址，向其发起请求，获取权限域名服务器的地址
   - 本地域名服务器根据这个权限域名服务器的地址，向其发起请求，最终得到该域名对应的 IP 地址
5. 本地域名服务器将得到的 IP 地址返回给主机，同时自己将 IP 地址缓存起来
6. 主机将 IP 地址返回给浏览器，同时自己也将 IP 地址缓存起
...
打开命令提示符：键入`ipconfig /displaydns`命令。

它将会列出所有当前存储在本地DNS缓存中的记录，包括域名、IP地址、记录类型等。

#### 清除本地DNS缓存

打开命令提示符：键入`ipconfig /flushdns`命令。

### DNS资源记录

DNS记录（Domain Name System records）用于存储和管理域名与相应网络资源之间的映射关系。DNS 是互联网中的一种分布式命名系统，它将易于记忆的域名（如`baidu.com`）映射到对应的`IP`地址（如`110.242.68.66`）或其他网络资源。

DNS记录的作用：域名解析、邮件交换、别名映射、文件记录、文本记录、加密证书

------

DNS记录常见的几种有：A、AAAA、CNAME、MX、NS、PTR、SOA

<strong style="color:#5090FF">A记录</strong>（Address Record，主机记录）：将域名映射到`IPv4`地址。

<strong style="color:#5090FF">AAAA记录</strong>：将域名映射到`IPv6`地址。

<strong style="color:#5090FF">CNAME记录</strong>（Canonical Name，别名记录）：用于创建域名的别名或指向另一个域名的指针。允许将一个域名映射到另一个域名，通常用于将子域名指向父域名。实现域名的别名和重定向。

> 注意：`CNAME` 记录只能指向另一个域名，不能直接指向 `IP` 地址。如果需要将域名指向 `IP` 地址，应使用 `A 记录`或 `AAAA 记录`。

<strong style="color:#5090FF">MX记录</strong>（邮件交换记录）：指定当前域名指向的邮件服务器。指定了邮件服务器的优先级和主机名，以便将电子邮件正确地路由到目标域名的邮件服务器。

简单邮件传输协议（SMTP）会根据MX记录的值来决定邮件的路由过程。

> 通常，`MX 记录`会配置多个邮件服务器，以提供冗余和负载均衡

<strong style="color:#5090FF">NS记录</strong>（域名服务器记录）：指定域名服务器的地址。用来指定该域名由哪个DNS服务器来进行解析。

<strong style="color:#5090FF">PTR记录</strong>（Pointer Record）：常被用于反向地址解析，将IP地址解析为域名

SOA记录（起始授权机构记录）：指定包含区域文件或 DNS 服务器的区域信息

SRV记录（服务定位资源记录）：提供一些特定的服务，例如主机和端口信息

TXT记录：以文本格式提供当前域名的信息。常用于验证域名的所有权和SPF邮件验证。

:book:参考：[参考文章](https://juejin.cn/post/6884183177926033416?searchId=20241024180303393DD1354D710BFB0C2A)、[参考文章](https://baijiahao.baidu.com/s?id=1667934625842583593&wfr=spider&for=pc)
