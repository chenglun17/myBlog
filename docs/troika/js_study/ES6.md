...
10. 新增 async、await，搭配 promise，可以通过编写形似同步的代码来处理异步流程, 提高代码的简洁性和可读性
...
14. 修饰器 @，decorator是一个函数，用来修改类甚至于是方法的行为。修饰器本质就是编译时执行的函数

## 2. var、let、const 之间的区别

> - var 是全局变量，存在变量提升过程，可以先使用在声明，可以重复声明
> - let 没有变量提升过程，必须先声明再使用，不能重复声明，但是可以重新赋值，存在暂时性死区
